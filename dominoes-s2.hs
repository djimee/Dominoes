...
playDomsRound p1 p2 seed
    = playDomsRound' p1 p2 P1 (hand1, hand2, [], (0,0))
      ...
      playDomsRound' p1 p2 turn gameState@(hand1, hand2, board, (score1,score2))
        ...
        | turn == P1 && p1_blocked = trace ("player 1 blocked " ++ show gameState) playDomsRound' p1 p2 P2 gameState
        | turn == P2 && p2_blocked = trace ("player 2 blocked " ++ show gameState) playDomsRound' p1 p2 P1 gameState
        | otherwise                = trace ("game state " ++ show gameState ++ "\n  player 1 plays " ++ show domino
                                        ++ " scoring " ++ show score ++ "\n  game state " ++ show newGameState)
                                        playDomsRound' p1 p2 P2 newGameState
          where
